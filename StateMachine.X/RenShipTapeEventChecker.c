...
uint8_t CheckRenShipTapeEvent(void){
    ...
    if (curRenShipTapeEvent != lastRenShipTapeEvent) { // check for change from last time
        thisEvent.EventType = curRenShipTapeEvent;
        returnVal = TRUE;
        lastRenShipTapeEvent = curRenShipTapeEvent; // update history
