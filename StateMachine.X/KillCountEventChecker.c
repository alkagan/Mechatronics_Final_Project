...
uint8_t CheckKillCountEvent(void){
    static ES_EventTyp_t lastKillCountEvent = CORNER_TAPE_NOT_DETECTED;
    ES_EventTyp_t curKillCountEvent = lastKillCountEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t checker_port_value = IO_PortsReadPort(PORTX & ALL_3_DESTROYED);

    if (checker_port_value == 0) { // is battery connected?
        curKillCountEvent = ALL_ATM6s_NOT_DESTROYED;
    } else if (checker_port_value == 1) {
        curKillCountEvent = ALL_ATM6s_DESTROYED;
    } else {
        curKillCountEvent = lastKillCountEvent;
    }
    if (curKillCountEvent != lastKillCountEvent) { // check for change from last time
        thisEvent.EventType = curKillCountEvent;
        returnVal = TRUE;
        lastKillCountEvent = curKillCountEvent; // update history
