...
uint8_t CheckCornerTapeEvent(void) {
    static ES_EventTyp_t lastCornerTapeEvent = CORNER_TAPE_NOT_DETECTED;
    ES_EventTyp_t curCornerTapeEvent = lastCornerTapeEvent;
    ES_Event thisEvent;
    ...
    if (curCornerTapeEvent != lastCornerTapeEvent) { // check for change from last time
        thisEvent.EventType = curCornerTapeEvent;
        returnVal = TRUE;
        lastCornerTapeEvent = curCornerTapeEvent; // update history
