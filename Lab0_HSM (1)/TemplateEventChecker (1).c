...
uint8_t CheckLight(void) {
    static enum {dark, light} CurrentLightState;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t CurrentLightValue = Roach_LightLevel(); // read the light level

    if (CurrentLightValue > LIGHT_THRESHOLD) { // is battery connected?
        CurrentLightState = dark;
    } 
    if (CurrentLightValue < LIGHT_THRESHOLD){
        CurrentLightState = light; 
    }
    if (CurrentLightState != LastLightState) { // check for change from last time
        thisEvent.EventType = LIGHT_EVENT;
    ...
    LastLightState = CurrentLightState; // update history
    return (returnVal);
}
